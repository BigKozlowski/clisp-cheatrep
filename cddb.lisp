...
(defun yn-prompt (prompt)
  (defun rerun-prompt (prompt)
    (format *query-io* "Please type y or n")
    (yn-prompt prompt))
  (format *query-io* prompt)
  (force-output *query-io*)
  (let ((input (read-line *query-io*)))
    (if (string-equal input "y") t
        (if (string-equal input "n") nil
            (rerun-prompt prompt)))))
...
(defun prompt-for-cd ()
  (make-cd
   ...
   (yn-prompt "Ripped [y/n]")))

(defun add-cds ()
  (loop (add-record (prompt-for-cd))
        (if (not (yn-prompt "Another? [y/n]: ")) (return))))
